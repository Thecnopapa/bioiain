    ...
    with torch.no_grad():
        ...
        for n, (tensor_path, entry, saprot_model) in enumerate(fs.saprot_embeddings(return_tensor=False)):
            ...
            if not DIFFERENCES:
                ...
                diff_diff_ax = fig.add_subplot(n_rows, n_figs, n_figs*2+5, projection="3d")
                diff_diff = diff_label_x + scaled_out
                voxels3d(diff_diff, ax = diff_diff_ax, shrink = True, title="Diff diff")


                abs_out_ax = fig.add_subplot(n_rows, n_figs+1, 5, projection="3d")
                abs_label_x_detached = abs_label_x.detach().cpu().numpy()[0]
                ...
                voxels3d(rel_out, ax = rel_out_ax, shrink = True, title="rel - (abs + out)")
